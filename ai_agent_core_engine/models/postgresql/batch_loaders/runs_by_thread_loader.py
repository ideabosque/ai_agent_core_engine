# -*- coding: utf-8 -*-
"""PostgreSQL DataLoader for RunModel records by thread_uuid (one-to-many).

Mirrors the DynamoDB RunsByThreadLoader contract:
.load(thread_uuid) returns a Promise that resolves to a **list** of normalized
run dicts for the given thread.
"""
...
class RunsByThreadLoader(SafeDataLoader):
    """Batch loader for RunModel records keyed by thread_uuid (returns lists)."""
    ...
    def _batch_load_fn(
        ...
        try:
            rows = (
                ...
                .filter(RunModel.thread_uuid.in_(unique_keys))
